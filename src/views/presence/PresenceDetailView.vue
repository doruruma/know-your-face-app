    ...
    <template v-if="!isLoading">
      ...
      <v-row>
        ...
        <v-col cols="12" lg="8" md="6" xs="12">
          ...
          <div class="mb-4">
            <div class="text-subtitle-1 font-weight-medium">Jarak dari kantor : </div>
...
const onClockInMapClick = () => {
  window.open(`https://maps.google.com/?q=${data.value.longitude_clock_in},${data.value.latitude_clock_in}`)
}

const onClockOutMapClick = () => {
  window.open(`https://maps.google.com/?q=${data.value.longitude_clock_out},${data.value.latitude_clock_out}`)
}
